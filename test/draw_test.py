...
class DrawLineTest(LineMixin, DrawTestCase):
    ...
    def _check_line_clipping(self, from_point, to_point, thickness):
        surf = self.surface
        ...
        # Get the expected points by drawing the line without the clipping
        # area being set.
        surf.set_clip(None)
        surf.fill(surf_color)
        pygame.draw.line(surf, line_color, from_point, to_point, thickness)
        expected_pts = get_color_points(surf, line_color, clip_rect)

        # Redraw on cleared surface with clipping area set.
        surf.fill(surf_color)
        surf.set_clip(clip_rect)
        pygame.draw.line(surf, line_color, from_point, to_point, thickness)

        surf.lock()  # For possible speed up.
    ...
    def test_line__shallow_angle_across_border(self):
        """Ensures draw.line (shallow angle) works correctly across borders.
    ...
    def test_line__steep_angle_across_border(self):
        """Ensures draw.line (steep angle) works correctly across borders.
    ...
    def test_line__thickness_grows_into_area(self):
        """Ensures draw.line just outside of clipping area is drawn correctly.

        This checks that lines matching the following description are drawn
        correctly:
           a line such that its centerline is outside of the clipping area
           while also being close enough so that due to its thickness, it
           grows across the border of the area.
        """

        self.surface = pygame.Surface((30, 30))
        self.clip_rect = pygame.Rect((10, 10), (11, 11))
        for p_set in ((12, 8, 19, 9), (8, 12, 9, 19), (22, 20, 18, 22)):
            for x_set in ((p_set[0], p_set[2]),
                          (30 - p_set[0], 30 - p_set[2])):
                for y_set in ((p_set[1], p_set[3]),
                              (30 - p_set[1], 30 - p_set[3])):
                    print(x_set[0], y_set[0], x_set[1], y_set[1])
                    thickness = 5
                    from_p = (x_set[0], y_set[0])
                    to_p = (x_set[1], y_set[1])
                    self._check_line_clipping(from_p, to_p, thickness)
